...
# X_train = train_data.drop(columns=['price', 'price_per_sqm', 'latitude', 'longitude'])
...
#
# # Calculate the correlation of each feature with price_per_sqm
# temp_data = X_train.copy()
# temp_data['price_per_sqm'] = y_train
# correlations = temp_data.corr()['price_per_sqm'].drop('price_per_sqm')
#
# # Normalize the correlations to [0, 1] for coloring
# norm = plt.Normalize(correlations.min(), correlations.max())
# colors = plt.cm.viridis((correlations.values))
#
...
# bars = ax.bar(range(X_train.shape[1]), importances[indices], color=colors[indices], align="center")
# ax.set_xticks(range(X_train.shape[1]))
# ax.set_xticklabels([X_train.columns[i] for i in indices], rotation=90)
# ax.set_xlim([-1, X_train.shape[1]])
# fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap='viridis'), ax=ax, label='Correlation with price_per_sqm')
# plt.show()
#
...
# scores = cross_val_score(rf_selected, X_train, y_train, cv=5, scoring=rmsle_scorer)
